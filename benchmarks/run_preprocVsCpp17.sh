...
####
declare -a BENCH_OPS=(
                "NANO_LOG(INFO, \"Starting backup replica garbage collector thread\");"
                "NANO_LOG(INFO, \"Opened session with coordinator at %s\", \"basic+udp:host=192.168.1.140,port=12246\");"
                "NANO_LOG(INFO, \"Backup storage speeds (min): %d MB/s read\", 181);"
                "NANO_LOG(INFO, \"buffer has consumed %lu bytes of extra storage, current allocation: %lu bytes\", 1032024, 1016544);"
                "NANO_LOG(INFO, \"Using tombstone ratio balancer with ratio = %0.6lf\", 0.400000);"
                "NANO_LOG(INFO, \"Initialized InfUdDriver buffers: %lu receive buffers (%u MB), %u transmit buffers (%u MB), took %0.1lf ms\", 50000, 97, 50, 0, 26.2);"
                "NANO_LOG(INFO, \"%p %s %d %s %p\", \"abc\", \"abc\", 50, \"abc\", \"abc\");  NANO_LOG(INFO, \"%d %p %s %p %d\", 50, \"abc\", \"abc\", \"abc\", 60);"
                )

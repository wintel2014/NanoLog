...
evilTestCase(NANO_LOG* log) {
    ...
    // Basic Tests
    ////////
    NANO_LOG(INFO, "Simple times");

    NANO_LOG(INFO, "More simplicity");

    NANO_LOG(INFO, "How about a number? %d", 1900);

    NANO_LOG(INFO, "How about a second number? %d", 1901);

    NANO_LOG(INFO, "How about three numbers without a space? %d%d%d", 1, 2, 3);

    NANO_LOG(INFO, "How about a double? %lf", 0.11);

    NANO_LOG(INFO, "How about a nice little string? %s", "Stephen Rocks!");

    NANO_LOG(INFO, "A middle \"%s\" string?", "Stephen Rocks!");

    NANO_LOG(INFO, "And another string? %s", "yolo swag! blah.");

    NANO_LOG(INFO, "One that should be \"end\"? %s", "end\0 FAIL!!!");

    int cnt = 2;
    NANO_LOG(INFO, "Hello world number %d of %d (%0.2lf%%)! This is %s!", cnt, 10, 1.0*cnt/10, "Stephen");
    ...
    NANO_LOG(INFO, "A const void* pointer %p", const_ptr);

    NANO_LOG(INFO, "I'm a small log with a small %s", "string");
    ...
    NANO_LOG(INFO, "Let's try out all the types! "
                "Pointer = %p! "
                "uint8_t = %u! "
                "uint16_t = %u! "
                "uint32_t = %u! "
    ...
    NANO_LOG(INFO, "how about some negative numbers? "
        "int8_t %d; "
        "int16_t %d; "
        "int32_t %d; "
        "int64_t %ld; "
    ...
    NANO_LOG(INFO, "How about variable width + precision? %*.*lf %*d %10s", 9, 2, 12345.12345, 10, 123, "end");

    NANO_LOG(INFO, "How about a variable length string that should end %.*s", 4, "here, but not here.");
    NANO_LOG(INFO, "And another one that should end %.4s", "here, but not here.");
...
// The Long string tests are commented out due to incompatibilities with certain versions of gcc.
    ...
    // NANO_LOG(WARN, "longString: %d %ls", 1, longString);
    ...
    // NANO_LOG(WARN, "NonConst %s ls=%ls s=%s",
    //          stringArray, longStringArray, stringArray);
    NANO_LOG(WARN, "NonConst %s and %s", stringArray, stringArray);
    NANO_LOG(WARN, "A Character %c", 'd');
    ...
    ++falsePositive;
    NANO_LOG(INFO, "10) NANO_LOG() \"NANO_LOG(\"Hi \")\"");
    ...
    // Joining of strings
    ////////
    NANO_LOG(INFO, "11) " "SD" "F");
    NANO_LOG(INFO, "12) NEW"
            "Lines" "So"
            "Evil %s",
            "NANO_LOG()");

    int i = 0;
    ++i; NANO_LOG(INFO, "13) Yup\n" "ieieieieieieie1"); ++i;
    NANO_LOG(INFO, "13.5) This should be =2: %d", i);


    ////////
    // Preprocessor's ability to rip out strange comments
    ////////
    NANO_LOG(INFO, "14) Hello %d",
        // 5
        5);
    ...
    int id = 0;
    NANO_LOG(INFO, "15) This should not be incremented twice (=1):%d", ++id);

    id++; NANO_LOG(INFO, "15) This should be incremented once more (=2):%d", id++); ++id;

    /* This */ NANO_LOG(INFO /* log */,  /* is */ "16) Hello /* uncool */");

    NANO_LOG(INFO, "17) This is " /* comment */ "rediculous");


    /*
     * NANO_LOG(INFO, "NANO_LOG");
     */
    ...
    ////////
     LOG(INFO, "sneaky #define LOG");
     hiddenInHeaderFilePrint();


    { NANO_LOG(INFO, "No %s", std::string("Hello").c_str()); }
    {NANO_LOG(INFO,
        "I am so evil"); }
    ...
    NANO_LOG(INFO, "%s", myString);

    const char *nonConstString = "Lol";
    NANO_LOG(INFO, "NonConst: %s", nonConstString);

    ////////
    // Strange Syntax
    ////////
    NANO_LOG(INFO, "{{\"(( False curlies and brackets! %d", 1);

    NANO_LOG(INFO, "Same line, bad form");      ++i; NANO_LOG(INFO, "Really bad")   ; ++i  ;
    ...
    NANO_LOG(INFO, "Make sure that the inserted code is before the ++i"); ++i;
...
CE, "The worse");
    ...
    NANO_LOG(INFO, "Ending on different lines"
    )
    ;


    NANO_LOG(INFO, "1) Simple times");
    ...
    NANO_LOG(INFO, "This string should end soon with 4 'a''s here: %.4s",
                static_cast<const char*>(largeString));

    int length = 5;
    NANO_LOG(INFO, "Another string that should end soon with 5 'a''s here: %.*s",
                length, static_cast<const char*>(largeString));

    NANO_LOG(INFO, "A string that's just one 'a': %.1000000s", "a");
    free(largeString);
}
...
void logLevelTest() {
    ...
    NANO_LOG(INFO, "Notice");
    NANO_LOG(WARN, "Warning");
